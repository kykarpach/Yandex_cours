...
public:
    // Порядок инициализации послк ":" важен - в каком порядке прописаны поля - в таком и тут они должны быть инициализированны...
    TicTacToe(size_t n, size_t k): N(n), K(k), Table(n, std::vector<int>(n, 0)), currentPlayer(1){
        // Table.reserve(N); --- БЫЛО ТАК И ПОЭТОМУ БЫЛА ПРОБЛTEМА, ТУТ ЛИШЬ ВЫДЕЛЯЕТСЯ ПАМЯТЬ  
    }
...
private:
    ...
    bool CheckDiagonal2(size_t i, size_t j) const{
        ...
        while (d1 <= i && d1 + j < N && Table[i - d1][j + d1] == Table[i][j]) {
            ++d1;
        }
